...
public class MyFileSystem implements FileSystem {
    ...
    private DirectBlock getDirectBlock(int fd, MODE mode) { //FIXME!!!
        ...
        int indirectBNum = IndirectBlock.COUNT;
        int DirectBPointer=0;//direct block pointer
        
        //directly point to the data block
        //use the direct block pointer to locate the position of the direct block
        if(blockNum <= 9) {
            ...
            //inode stores the pointers pointed to direct block
            DirectBPointer=inode.ptr[blockNum];
            return new DirectBlock(disk, DirectBPointer, blockOff, fresh);
        }else if (blockNum < 10 + indirectBNum){ 
        	
        	//singly indirect block
        	//fresh would be true if there's no indirect blocks
        	boolean fresh = inode.ptr[10] == 0;
            IndirectBlock indirectB = new IndirectBlock();
            
            if(fresh){ //fresh is true, meaning, there's no indirect block
                if(mode == MODE.r)
                    return DirectBlock.hole;
                else if((inode.ptr[10] = freeMap.find()) == 0){ 
                    return null;
                }
            } else { //if indirect block exits, then read the indirect block
            	disk.read(inode.ptr[10], indirectB);
            }
            //single indirect block pointer
            //if it's 0, meaning, the first indirect block
            int indirectPointer = blockNum - 10;
            return getDirectBPointedBySinglyIndirectB(indirectB, indirectPointer, blockOff, mode, inode.ptr[10]);
        }else if (blockNum < 10 + indirectBNum * (1+indirectBNum)) {
        	
        	//doubly indirect block
        	
        	int indirectPointer1 = (blockNum - 10 - indirectBNum) / indirectBNum;//first pointer of doubly indirect block
            int indirectPointer2 = (blockNum - 10 - indirectBNum) % indirectBNum;//second pointer of doubly indirect block
            ...
            if(fresh){ //fresh is true, meaning, there's no indirect block
            	...
            	disk.read(inode.ptr[11], indirectB);
            }
            
            return getDirectBPointedByDoublyIndirectB(indirectB, indirectPointer1, blockOff, mode, inode.ptr[11], indirectPointer2);
            
        } else if (blockNum < 10 + indirectBNum * (1 + indirectBNum + indirectBNum*indirectBNum )) {
        	
        	//triply indirect block
        	
        	//indirect Pointers
        	//at most need three pointers to locate the position of indirect block
        	int indirectPointer1 = (blockNum - 9 - indirectBNum - indirectBNum*indirectBNum) / (indirectBNum * indirectBNum);
            int indirectPointer2= ((blockNum - 9 - indirectBNum - indirectBNum*indirectBNum) % (indirectBNum * indirectBNum)) / indirectBNum;
            int indirectPointer3 = ((blockNum - 9 - indirectBNum - indirectBNum*indirectBNum) % (indirectBNum * indirectBNum)) % indirectBNum;
            ...
            if(fresh){ //fresh is true, meaning, there's no indirect block
            	...
            	disk.read(inode.ptr[12], indirectB);
            }
            
            int diskPosition=indirectB.ptr[indirectPointer1];
        	fresh = indirectB.ptr[indirectPointer1] == 0;
            IndirectBlock indirectB2 = new IndirectBlock();
            
            if(fresh){ //if there is no IndirectBlock
                if(mode == MODE.r)
                    return DirectBlock.hole;
                else if((indirectB.ptr[indirectPointer1] = freeMap.find()) == 0)
                    return null;
                
                disk.write(inode.ptr[12], indirectB);//write the indirect block on block
                diskPosition = indirectB.ptr[indirectPointer1];//update the position
            } else { //if indirect block exits, then read the indirect block
            	disk.read(indirectB.ptr[indirectPointer1], indirectB2);
            }           
        	return getDirectBPointedByDoublyIndirectB(indirectB2, indirectPointer2, blockOff, mode, diskPosition, indirectPointer3);           
        } else {
    ...
    */    
    ...
    private DirectBlock getDirectBPointedBySinglyIndirectB(IndirectBlock indirectB, int indirectPointer, int blockOff, MODE mode, int diskPosition) {	
    	boolean fresh = indirectB.ptr[indirectPointer] == 0;
    	if(fresh)
            if(mode == MODE.r)
                return DirectBlock.hole;
            else if((indirectB.ptr[indirectPointer] = freeMap.find()) == 0)
                return null;   	  
    	disk.write(diskPosition, indirectB);//write the indirect block on disk
        return new DirectBlock(disk, indirectB.ptr[indirectPointer], blockOff, fresh);
    }
    
    
    /** @param   indirectB          
     * @param   indirectPointer1        
     * @param   blockOff
     * @param   mode
     * @param   diskPosition 
     * @param   indirectPointer2                
     * @returns DirectBlock 
     *                      
     */      
    private DirectBlock getDirectBPointedByDoublyIndirectB(IndirectBlock indirectB, int indirectPointer1, int blockOff, MODE mode, int diskPosition, int indirectPointer2) {    	
    	boolean fresh = indirectB.ptr[indirectPointer1] == 0;
        IndirectBlock indirectB2 = new IndirectBlock();
        if(fresh){ //if there is no IndirectBlock
            if(mode == MODE.r){
                return DirectBlock.hole;
            ...
            disk.write(diskPosition, indirectB);
        } else { //if indirect block exits, then read the indirect block
        	disk.read(indirectB.ptr[indirectPointer1], indirectB2);
        }
    	return getDirectBPointedBySinglyIndirectB(indirectB2, indirectPointer2, blockOff, mode, indirectB.ptr[indirectPointer1]);
    }
